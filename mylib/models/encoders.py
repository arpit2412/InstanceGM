...
__all__ = ["CONV_Decoder_ANIMAL","CONV_Encoder_ANIMAL", "CONV_T_Decoder_ANIMAL", "CONV_Decoder_FMNIST","CONV_Encoder_FMNIST", "CONV_T_Decoder","Y_Encoder","Z_Encoder", "X_Decoder", "T_Decoder","CONV_Encoder_CIFAR","CONV_Decoder_CIFAR", "CONV_Encoder_CLOTH1M","CONV_Decoder_CLOTH1M","CONV_T_Decoder_CLOTH1M", "CONV_Encoder_WEBVISION","CONV_Decoder_WEBVISION","CONV_T_Decoder_WEBVISION"]
...
class CONV_Encoder_ANIMAL(nn.Module):
    def __init__(self, in_channels =3, feature_dim = 32, num_classes = 2,  hidden_dims = [32, 64, 128, 256], z_dim = 2):
        ...
        self.z_dim = z_dim
        self.feature_dim = feature_dim
        self.embed_class = nn.Linear(num_classes, feature_dim * feature_dim) #num_classes
        self.embed_data = nn.Conv2d(in_channels, in_channels, kernel_size=1)
        self.in_channels = in_channels
        in_channels += 1
        modules = []
...
class CONV_T_Decoder_ANIMAL(nn.Module):
    def __init__(self, in_channels =1, feature_dim = 64, num_classes = 10,  hidden_dims = [32, 64, 128, 256]):
        super().__init__()
...
class CONV_Encoder_WEBVISION(nn.Module):
    def __init__(self, in_channels =3, feature_dim = 299, num_classes = 2,  hidden_dims = [32, 64, 128, 256, 512], z_dim = 2):
        super().__init__()
        self.z_dim = z_dim
        self.feature_dim = feature_dim
        self.embed_class = nn.Linear(num_classes, feature_dim * feature_dim)
